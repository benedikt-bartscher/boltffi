...
C supports synchronous callback traits through a vtable and a typed handle. Its callback method signatures still use bridge types; see the [C vtable example](#c-vtables). Inline closures and async callback traits are not generated for C.
...
This example uses `demo.h` from the [demo library](https://github.com/boltffi/boltffi/tree/main/examples/demo). The callback uses its identity as an integer offset, so it owns no allocated state:
