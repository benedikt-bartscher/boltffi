...
This example uses the [demo library's Email type](https://github.com/boltffi/boltffi/tree/main/examples/demo/src/custom_types). A scalar-backed custom type is an alias for that scalar and needs no cleanup. See [memory management](/docs/c#memory-management) for borrowed views and returned owners.
