...
Functions can accept callback traits. A callback trait is a Rust trait marked with `#[export]` that the target language implements. Unlike closures (single anonymous function), callback traits can have multiple methods and carry state. C supports synchronous callback traits with the [C vtable contract](/docs/callbacks#c-vtables); inline closures are not generated for C.
...
C does not generate inline closure parameters yet. Synchronous callback traits have a [C vtable API](/docs/callbacks#c-vtables).
