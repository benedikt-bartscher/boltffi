...
setuptools' `build/` and `*.egg-info` directories are removed before each wheel build, so every wheel is built from the current sources. A wheel missing the compiled `_native` extension or the shared library fails the pack.
