...
This builds every ABI listed under `targets.android.architectures` (by default arm64-v8a, armeabi-v7a, x86, and x86_64), generates Kotlin bindings and JNI glue, and copies the shared libraries to jniLibs. When `[targets.android.kotlin.desktop_pack]` is enabled with the bundled desktop loader, it also packs the Kotlin desktop natives.
...
To split the work across parallel CI jobs, pack one slice per job:

```bash
boltffi pack android --release --architecture arm64 --skip-desktop
boltffi pack android --release --architecture x86_64 --skip-desktop
boltffi pack android --release --desktop-only
```

- `--architecture <ARCH>` builds only that architecture and can be repeated. Accepted values are `arm64`, `armv7`, `x86_64`, and `x86`. Naming an architecture that isn't configured under `targets.android.architectures` is an error.
- `--skip-desktop` skips the Kotlin desktop natives even when the configuration enables them.
- `--desktop-only` builds only the Kotlin desktop natives and leaves jniLibs as they are. It can't be combined with `--architecture` or `--skip-desktop`, and it's an error unless desktop native packaging is enabled.

Slices can be packed into the same `dist/` one after another. A run only removes jniLibs for ABIs that are no longer configured, so it keeps the ABIs that other slices packaged. When a run packs only some of the configured ABIs, it writes one debug symbols archive per ABI, `{crate_artifact_name}.android.{abi}.symbols.zip`, instead of the combined archive, and leaves any existing combined archive alone. A later run that packs every configured ABI writes the combined archive and removes those per-ABI archives.
...
├── include/mylib.h
└── lib/
    ├── shared library and Windows import library
    ├── static/
    │   └── static library
    ├── cmake/mylib/
    │   ├── mylibConfig.cmake
...
| --- | --- | --- | --- |
| Linux | `libmylib.so` | `static/libmylib.a` | Not needed |
| macOS | `libmylib.dylib` | `static/libmylib.a` | Not needed |
| Windows, MSVC | `mylib.dll` | `static/mylib.lib` | `mylib.dll.lib` |
| Windows, GNU | `mylib.dll` | `static/libmylib.a` | `libmylib.dll.a` |
