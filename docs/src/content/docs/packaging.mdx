...
`getrandom`, which `rand`, `uuid`'s `v4` and most other crates draw entropy
from, has no default backend on `wasm32-unknown-unknown`, and its `wasm_js`
backend needs `wasm-bindgen`'s generated glue, which a BoltFFI package does not
have. BoltFFI provides the entropy instead: `@boltffi/runtime` fills wasm memory
straight from `crypto.getRandomValues`, and the `getrandom` feature connects
`getrandom` 0.4 to it.

Enable the feature and install the backend once, in the crate you pack:
...
Then select `getrandom`'s custom backend for the wasm target, in the crate's
`.cargo/config.toml`:
...
The macro expands to nothing on other targets, which keep their usual source.
`getRandomValues` is available in browsers, workers and Node.js 19+; where it is
missing, `getrandom` returns `Error::UNSUPPORTED` rather than trapping. Code that
needs a lot of randomness should seed a userspace generator such as `rand`'s
`StdRng` from `getrandom` once rather than calling it per value, as it would on
any other platform.
