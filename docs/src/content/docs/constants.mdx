...
C examples use the package name `demo` and include `demo.h`. See [C memory management](/docs/c#memory-management) for borrowed inputs and returned owners.
...
The Rust name is converted to the naming convention of each target. `API_VERSION` becomes `apiVersion` in Swift, Kotlin, and TypeScript, `API_VERSION` in Java, `ApiVersion` in C#, and `api_version` in Python. In a C package named `demo`, it becomes `DEMO_API_VERSION`.
...
Values such as `Color { ... }` are constructed in Rust and delivered through a generated accessor. The generated API still exposes an ordinary immutable value. C emits simple constants as package-prefixed macros. Constants that require Rust evaluation use accessor functions; owning values returned by those accessors need cleanup just like function results.
