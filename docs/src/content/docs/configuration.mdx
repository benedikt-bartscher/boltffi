...
The generated `setup.py` declares `python_requires=">=3.10"` by default. Raise it when your package needs a newer Python.
...
Declare console scripts under `[targets.python.scripts]`. Each key is the command name and each value is an entry point in `module:function` form.
...
The wheel includes every regular package under the module directory, so subpackages you add next to the generated sources, such as `demo_runtime/cli/__init__.py`, ship with it together with their `.pyi` stubs and `py.typed` markers.
