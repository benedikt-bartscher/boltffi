...
The `[cargo]` section adds arguments to the `cargo` invocations BoltFFI runs. `global_args` apply to every invocation, and `command_args` apply per BoltFFI command:
...
When your crate gates bindings behind Cargo features, set `cargo_args` on a target table instead. Those arguments apply only where that target builds, for example `boltffi generate python` and `boltffi pack python`:
...
Arguments are passed in this order: `global_args`, `command_args`, the target's `cargo_args`, then any `--cargo-arg` values from the command line.
...
The header, CMake package, and pkg-config files use `package.crate_name`, falling back to `package.name`. Native binaries keep the selected Cargo library target's name. Package metadata uses `package.version` when set, otherwise the selected Cargo package's version.
