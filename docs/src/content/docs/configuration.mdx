...
`boltffi pack android` writes `{crate_artifact_name}.android.symbols.zip` to that directory. The archive mirrors the `jniLibs/<abi>/` layout and includes a `symbols.json` manifest with ABI and target mappings. A run that packs only some of the configured ABIs with `--architecture` writes one `{crate_artifact_name}.android.{abi}.symbols.zip` per ABI instead.
