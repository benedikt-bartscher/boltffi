...
A variant whose payload is exactly one record can be marked
`#[boltffi::transparent]`. The bindings then drop the wrapper class for that
variant: the payload record itself becomes the variant, so reading the enum
means one less allocation and one less field hop, and a type check against
the payload record smart-casts directly.
...
- The payload must be exactly one record. Unit variants, scalar payloads,
  and multi-field payloads keep their wrapper classes (mixing transparent
  and wrapped variants in one enum is fine).
- No two transparent variants of one enum may carry the same payload
  record: encode recovers the tag from the payload's concrete type, which a
  shared record would make ambiguous. Note the flip side: adding a second
...
- Python additionally requires the payload record to be encoded (a record
  containing only fixed-width primitives is a C-native class there and
  cannot inherit the enum class).
