...
`Option<T>` becomes nullable in the managed languages. In C, an optional field has `has_value` and `value`; read `value` only when `has_value` is true.
...
C methods take a pointer and update the record or enum in place. If it has owned fields, pass an owning value returned by the generated API. The method replaces old fields and releases their allocations. Do not fill those fields with string literals or borrowed pointers before passing the value to a mutable method.
