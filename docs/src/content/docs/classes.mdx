...
C examples use the package name `demo` and include `demo.h`. Printing examples also include `<stdio.h>` and, for `PRIu64`, `<inttypes.h>`. See [C memory management](/docs/c#memory-management) for borrowed inputs and returned owners.
...
Constructors can return `Result<Self, E>`. A custom error type must be marked with `#[error]`. The managed targets expose a throwing constructor. C returns a typed result: check `ok` before using `data.value`, and release the result when finished. Its cleanup function also releases the returned class handle; do not free the handle separately.
...
C callers release a class with its generated `<package>_<class>_free` function. The handle's fields are private implementation details; pass its address to generated methods. A C struct assignment does not retain the Rust object, so keep one owner. A borrowed class parameter does not take ownership; a consuming parameter takes a mutable pointer and clears the handle after the call. The Rust class's threading restrictions also apply to C callers.

In every target, keeping a class handle alive also keeps its Rust object alive.
